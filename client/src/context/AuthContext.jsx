...
export function AuthProvider({ children }) {
  ...
  const login = async (email, password) => {
    try {
      ...
      return { ok: false, error: err.response?.data?.message || 'Could not connect to server. Is the backend running?' }
    }
  }
  ...
  const register = async (name, email, password) => {
    try {
      ...
      return { ok: false, error: err.response?.data?.message || 'Could not connect to server.' }
    }
  }
