...
export default function Profile() {
  ...
  return (
    <div className="anim-up">
      {/* className="profile-header" makes it column on mobile */}
      ...
      >
        ...
        <div>
          ...
          <div style={{
            display: 'inline-flex', alignItems: 'center', gap: 6, marginTop: '0.5rem',
            background: 'rgba(124,58,237,0.15)', border: '1px solid var(--border2)',
            borderRadius: 20, padding: '4px 12px', fontSize: '0.8rem', fontWeight: 600, color: 'var(--violet2)',
          }}>
            ⚡ Level {level} · {xp} XP
          </div>
        </div>
      ...
      {/* className="profile-stats" makes it 2-col on mobile */}
      ...
      <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 16, padding: '1.5rem', marginBottom: '1.5rem' }}>
        <h3 style={{ fontFamily: 'Sora,sans-serif', fontSize: '0.95rem', fontWeight: 700 }}>XP Progress</h3>
        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.8rem', color: 'var(--muted)', margin: '0.75rem 0' }}>
          <span>Level {level}</span>
          <span>Level {level + 1}</span>
        </div>
        <div style={{ height: 12, background: 'rgba(255,255,255,0.06)', borderRadius: 10, overflow: 'hidden' }}>
          <div style={{ height: '100%', width: `${xpPct}%`, background: 'linear-gradient(90deg,var(--violet),var(--mint))', transition: 'width 1s ease' }} />
        </div>
        <div style={{ textAlign: 'center', marginTop: '0.75rem', fontSize: '0.875rem', color: 'var(--muted)' }}>
      ...
      <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 16, padding: '1.5rem' }}>
        <h3 style={{ fontFamily: 'Sora,sans-serif', fontSize: '0.95rem', fontWeight: 700 }}>Account</h3>
        <button onClick={handleLogout} style={{
          marginTop: '1rem', padding: '0.6rem 1.25rem', borderRadius: 10,
          background: 'rgba(244,63,94,0.12)', color: '#FB7185',
          border: '1px solid rgba(244,63,94,0.3)', fontSize: '0.875rem',
...
function StatCard({ label, value, accent }) {
  return (
    <div style={{
      ...
      <div style={{ position: 'absolute', top: 0, right: 0, width: 60, height: 60, borderRadius: '50%', background: accent, opacity: 0.08, transform: 'translate(12px,-12px)' }} />
